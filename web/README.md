...
PirateBay, Nyaa, SubsPlease, and FitGirl have no CORS headers, so their requests go through a configurable, ordered list of proxies (`corsproxy.io`, then `api.allorigins.win`, by default) — each source tries them in order and falls through to the next on failure. **Free public proxies have no uptime guarantee**; nearly 20 candidates were tested (including the full list from [this gist](https://gist.github.com/jimmywarting/ac1be6ea0297c16c477e17f8fbe51347)) and most turned out dead, blocklisted for torrent domains, or just docs pages with no live endpoint behind them. `corsproxy.io` is fast and reliable for three of the four sources but blocklists `nyaa.si` specifically (`403 This domain has been blocked`); `allorigins.win` covers that gap but is noticeably slower (sometimes 8–12s) and occasionally times out on its own — hence both being defaults, tried in that order. Open Settings (⚙) to add your own as a fallback (one URL prefix per line, tried top to bottom) if search for those sources stops working.
...
- **CORS-proxied sources (Nyaa, SubsPlease, FitGirl, PirateBay) do not work via `file://`** — `api.allorigins.win` doesn't grant `Origin: null` (what `file://` sends) the same access it grants a real origin like `http://localhost`; this hasn't been re-verified against `corsproxy.io` specifically, but the underlying `null`-origin restriction is a common policy among free proxies, not something fixable in this app's code either way.
